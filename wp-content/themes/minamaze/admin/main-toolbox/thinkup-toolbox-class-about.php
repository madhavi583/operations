 ...
 * @since 1.0.0
 */
if ( ! class_exists( 'minamaze_thinkup_about_page' ) ) {
	/**
	 * Singleton class used for generating the about page of the theme.
	 */
	class minamaze_thinkup_about_page {
		/**
		 * Define the version of the class.
		 *
		 * @var string $version The minamaze_thinkup_about_page class version.
		 */
		...
		/**
		 * The single instance of minamaze_thinkup_about_page
		 *
		 * @var minamaze_thinkup_about_page $instance The  minamaze_thinkup_about_page instance.
		 */
		private static $instance;

		/**
		 * The Main minamaze_thinkup_about_page instance.
		 *
		 * We make sure that only one instance of minamaze_thinkup_about_page exists in the memory at one time.
		 *
		 * @param array $config The configuration array.
		 */
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof minamaze_thinkup_about_page ) ) {
				self::$instance = new minamaze_thinkup_about_page;
		...
		function register() {
			if ( ! empty( $this->menu_name ) && ! empty( $this->page_name ) ) {
				add_theme_page( $this->menu_name, $this->page_name, 'activate_plugins', 'thinkup-welcome', array(
					$this,
					'minamaze_thinkup_about_page_render',
				) );
			}
		}
		...
		public function activation_admin_notice() {
			...
			add_action( 'admin_notices', array( $this, 'minamaze_thinkup_about_page_welcome_admin_notice' ), 99 );
		}

		/**
		 * Display an admin notice linking to the about page
		 */
		public function minamaze_thinkup_about_page_welcome_admin_notice() {
			if ( ! empty( $this->notification ) ) {
				...
				if ( current_user_can( 'edit_theme_options' ) && !get_option( 'minamaze_thinkup_notice_welcome' ) ) {
					...
					echo '<a class="notice-dismiss" href="' . esc_url(wp_nonce_url(remove_query_arg(array('activated'), add_query_arg('minamaze-thinkup-hide-notice', 'welcome')), 'minamaze_thinkup_hide_notices_nonce', '_minamaze_thinkup_notice_nonce')) . '" style="z-index: 9;padding: 10px;text-decoration: none;" >';
		 ...
		 */
		public function minamaze_thinkup_about_page_render() {
		...
		public function style_and_scripts( $hook_suffix ) {
			...
			if ( 'themes.php' === $pagenow || 'appearance_page_thinkup-welcome' == $hook_suffix ) {
				...
				wp_enqueue_style( 'minamaze-thinkup-about-page-css', get_template_directory_uri() . '/admin/main-toolbox/assets/css/thinkup-toolbox-backend.css' );
 ...
 */
if (!function_exists('minamaze_thinkup_hide_notice')) {
	function minamaze_thinkup_hide_notice() {
...
add_action('wp_loaded', 'minamaze_thinkup_hide_notice');